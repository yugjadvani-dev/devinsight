...
title: Dynamic OG image generation in DevInsight blog posts
slug: dynamic-og-image-generation-in-DevInsight-blog-posts
featured: false
draft: false
tags:
  - docs
  - release
description: New feature in DevInsight v1.4.0, introducing dynamic OG image generation for blog posts.
---

New feature in DevInsight v1.4.0, introducing dynamic OG image generation for blog posts.
...
DevInsight already provided a way to add an OG image to a blog post. The author can specify the OG image in the frontmatter `ogImage`. Even when the author doesn't define the OG image in the frontmatter, the default OG image will be used as a fallback (in this case `public/DevInsight-og.jpg`). But the problem is that the default OG image is static, which means every blog post that does not include an OG image in the frontmatter will always use the same default OG image despite each post title/content being different from others.
...
In DevInsight v1.4.0, Vercel's [Satori](https://github.com/vercel/satori) package is used for dynamic OG image generation.
...
## Anatomy of DevInsight dynamic OG image

Dynamic OG image of DevInsight includes _the blog post title_, _author name_ and _site title_. Author name and site title will be retrieved via `SITE.author` and `SITE.title` of **"src/config.ts"** file. The title is generated from the blog post frontmatter `title`.  
